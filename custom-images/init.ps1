...
# CD_HOST
Set-EnvFileVariable "CD_HOST" -Value "cd.$($HostName)"

# CM_HOST
Set-EnvFileVariable "CM_HOST" -Value "cm.$($HostName)"

# ID_HOST
Set-EnvFileVariable "ID_HOST" -Value "id.$($HostName)"

# HRZ_HOST
Set-EnvFileVariable "HRZ_HOST" -Value "hrz.$($HostName)"
...
try {
    ...
    & $mkcert -install
    & $mkcert -key-file "$($HostName).key.pem" -cert-file "$($HostName).cert.pem" "*.$($HostName)"
}
catch {
...
Add-HostsEntry "cd.$($HostName)"
Add-HostsEntry "cm.$($HostName)"
Add-HostsEntry "id.$($HostName)"
Add-HostsEntry "hrz.$($HostName)"
